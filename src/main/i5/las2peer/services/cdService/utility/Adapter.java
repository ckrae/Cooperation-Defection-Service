...
*
* Adapter
* Get the graphs from the OCD Service and
* Transform them into MASON compatibility networks
* 
*/
public final class Adapter {
	...
	public static Network getGraphById(String graphIdStr) throws ServiceNotFoundException, ServiceNotAvailableException, RemoteServiceException {
	...
	public static Network getGraphById(long graphId) throws ServiceNotFoundException, ServiceNotAvailableException, RemoteServiceException {	

	Network network = new Network(false);
	
	if(CDService.USE_GRAPH_RMI)	{		

		HashMap<String, Object> map = (HashMap<String, Object>) Context.getCurrent().invoke(graphService, "getGraphById", graphId);
	...
	/*
	public static SimulationSeries createSimulationSeries(Game gameint iterations) {
		 
		 /// Create Simulation Series
		 SimulationSeries series = new SimulationSeries(game.getGraphId() , game.getPayoffScheme().toDoubleArray(), game.getDynamic().toString(), game.getIterations());
		 for ( int i = 1; i < iterations; i++ ) {
			 
			 SimulationData sim = game.simulate();
			 series.addSimnulationData(sim);
		 }		 
